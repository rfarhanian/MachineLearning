...
class DataVisualizer:
    ...
    @classmethod
    def visualize_cluster(cls, matrix, labelList, cluser_size=3):
        ...
        kmeans = KMeans(n_clusters=cluser_size)
        kmeans.fit(matrix)
    ...
    def heat_map(self, results, restaurant: Restaurant, people: People):
        # https://seaborn.pydata.org/generated/seaborn.heatmap.html
        plot_dims = (12, 10)
        fig, ax = plt.subplots(figsize=plot_dims)
        sns.heatmap(ax=ax, data=results, annot=True, yticklabels=restaurant.get_names(), xticklabels=people.get_names())
        plt.show()
